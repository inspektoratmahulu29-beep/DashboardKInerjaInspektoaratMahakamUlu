...
function Dashboard({derived,year,onNav,onSheet}){
  ...
  return <section className="page dashboard-page">
    ...
    <div className="kpi-grid">{primary.map((x,i)=><KPI key={i} {...x} onClick={()=>setKpiDetail(x.t)}/>)}</div>
    ...
    {kpiDetail && <KPIDetailModal type={kpiDetail} derived={derived} onClose={()=>setKpiDetail(null)} onNav={onNav} onSheet={onSheet}/>}
  </section>
}

function KPI({t,v,s,p,tone,onClick}){return <button type="button" className={`kpi ${tone}`} onClick={onClick}><div className="shine"></div><div className="kpi-top"><span>{t}</span><i>↗</i></div><strong title={v}>{v}</strong><small>{s}</small>{p!==undefined&&<div className="meter"><i style={{width:`${Math.max(0,Math.min(100,p||0))}%`}}></i></div>}</button>}

function KPIDetailModal({type,derived,onClose,onNav,onSheet}){
  const cfg={
    'Total Anggaran':{ey:'KEUANGAN • DETAIL',title:'Total Anggaran',accent:'gold',value:money(derived.budgetTotal),desc:'Akumulasi seluruh nilai anggaran valid dari sheet Realisasi Fisik & Keu.',stats:[['Sumber','Realisasi Fisik & Keu'],['Baris valid',number(derived.physicalRowsCount)],['Serapan',pct(derived.financialRate)],['Sisa dana',money(Math.max(0,derived.budgetTotal-derived.financialTotal))]],action:'Buka sumber realisasi',sheet:'Realisasi Fisik & Keu'},
    'Realisasi Keuangan':{ey:'KEUANGAN • REALISASI',title:'Realisasi Keuangan',accent:'blue',value:money(derived.financialTotal),desc:'Akumulasi realisasi keuangan yang bernilai numerik dan dapat diverifikasi.',stats:[['Anggaran',money(derived.budgetTotal)],['Serapan',pct(derived.financialRate)],['Sisa dana',money(Math.max(0,derived.budgetTotal-derived.financialTotal))],['Status','Data valid']],action:'Buka sumber realisasi',sheet:'Realisasi Fisik & Keu'},
    'Serapan Keuangan':{ey:'KEUANGAN • SERAPAN',title:'Serapan Keuangan',accent:'mint',value:pct(derived.financialRate),desc:'Rasio realisasi keuangan terhadap anggaran. Semakin mendekati 100%, semakin tinggi serapan.',stats:[['Rumus','Realisasi ÷ Anggaran × 100'],['Anggaran',money(derived.budgetTotal)],['Realisasi',money(derived.financialTotal)],['Sisa',money(Math.max(0,derived.budgetTotal-derived.financialTotal))]],action:'Analisis realisasi',sheet:'Realisasi Fisik & Keu'},
    'Rata-rata Fisik':{ey:'KINERJA • FISIK',title:'Rata-rata Realisasi Fisik',accent:'teal',value:pct(derived.physicalRate),desc:'Rata-rata seluruh nilai Realisasi Fisik (%) yang lolos validasi angka.',stats:[['Sampel valid',number(derived.physicalRowsCount)],['Rata-rata',pct(derived.physicalRate)],['Serapan keuangan',pct(derived.financialRate)],['Basis','Kolom Realisasi Fisik']],action:'Lihat kertas kerja',sheet:'Realisasi Fisik & Keu'},
    'Penugasan PKPT':{ey:'PENUGASAN • PKPT',title:'Progress Penugasan PKPT',accent:'violet',value:number(derived.pkptCount),desc:'Jumlah penugasan yang teridentifikasi dalam Rekap realisasi PKPT.',stats:[['Selesai',number(derived.done)],['Berjalan',number(derived.progress)],['Belum',number(derived.pending)],['Total',number(derived.pkptCount)]],action:'Buka rekap PKPT',sheet:'Rekap realisasi PKPT'},
    'Output Utama':{ey:'MONITORING • OUTPUT',title:'Output Subkegiatan Utama',accent:'pink',value:number(derived.outputUtama),desc:'Jumlah aktivitas output utama yang terdeteksi pada sheet monitoring output.',stats:[['Output utama',number(derived.outputUtama)],['Output penunjang',number(derived.outputPenunjang)],['Total',number(derived.outputUtama+derived.outputPenunjang)],['Sumber','Monev output Subkegiatan Utama']],action:'Buka sumber output',sheet:'Monev output Subkegiatan Utama'}
  }[type];
  if(!cfg)return null;
  const progress=(type==='Penugasan PKPT'?derived.pkptCount?derived.done/derived.pkptCount*100:0:type==='Serapan Keuangan'?derived.financialRate:type==='Rata-rata Fisik'?derived.physicalRate:derived.financialRate);
  return <div className="modal-backdrop kpi-modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><div className={`kpi-modal ${cfg.accent}`}><div className="kpi-modal-aura"></div><div className="kpi-modal-head"><div><span className="eyebrow">{cfg.ey}</span><h2>{cfg.title}</h2><p>{cfg.desc}</p></div><button className="close" onClick={onClose}>×</button></div><div className="kpi-modal-hero"><div className="kpi-modal-value">{cfg.value}</div><div className="kpi-modal-orbit"><div className="pulse-ring"></div><div className="pulse-core">↗</div></div></div><div className="kpi-detail-grid">{cfg.stats.map((x,i)=><div key={i}><small>{x[0]}</small><b>{x[1]}</b></div>)}</div><div className="kpi-progress"><div><span>Indeks capaian</span><b>{pct(progress)}</b></div><div className="progress-track"><i style={{width:`${Math.max(0,Math.min(100,progress||0))}%`}}></i></div></div><div className="kpi-modal-foot"><button className="soft" onClick={()=>{onClose();onSheet(cfg.sheet)}}>▦ {cfg.action}</button><button className="primary" onClick={()=>{onClose();onNav('editor')}}>✎ Edit data sumber</button></div></div></div>
}
